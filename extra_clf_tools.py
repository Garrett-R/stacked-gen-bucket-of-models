...
# TODO: running this with Python 3, I have introduced division problems?
# in other words, was I previously doing any integer division?
...
def test_feat_perf(data, target,
    ...
    if cv_eval == None:
        ...
        if smallest_cl_size < default_num_folds:
            print("Warning: the smallest class of your data has just " + str(smallest_cl_size) + " datapoints which is insufficient for the default cross-validation (which requires at least " + str(default_num_folds) + " datapoints).")
            print("I've reduced the number of folds in the cross-validation from " + str(default_num_folds) + " to " + str(smallest_cl_size) + ", so it should be fine.")
    ...
    for train_indices, test_indices in cv_eval:
        ...
        #ERROR-CHECKING: see if par_n_folds is larger than number of elements in smallest class of train_target
        ...
        if par_n_folds > smallest_cl_size:
            ...
            print("Warning: the parameter par_n_folds was too big, so I reduced it to be: " + str(par_n_folds) + ".")
        ...
        ##########
        #These next three commands train on classifier then make prediction on test set
        ##########

        #find the best classifier and parameters for this training set
        best_clf = find_best_clf(train_data, train_target, clf_list, iterations, cv_par_est, verbose)
        ...
        if verbose:
            print("The performance for the current " + str(int(len(test_target))) + " holdout datapoints was: " + str(round(np.mean(temp_scores)*100, 1)) + "%\n")
...
def find_best_clf(train_data, train_target, clf_list=['svm_rbf', 'svm_linear', 'svm_poly'],
                                            iterations=3, cv=None, par_n_folds=10, verbose=False, probability=False):
    """Finds the best classifier with optimal parameters by using an adaptive grid search.

    This uses cross-validation to find best parameters on a logarithmic parameter grid, and
    then searches again on smaller logarithmic grid centered at previous best parameter set.

    INPUT:
        >train_data: the training data, a (num_subjects, num_features) shaped NumPy array
        >train_target: targets, a (num_subjects) shaped NumPy array
        >clf_list: a list of the names of classifiers that should be considered.  Possible names:
            'all', 'svm_rbf', 'svm_linear', 'svm_poly', 'svm_sigmoid', 'log_reg', 'random_forest'
        >iterations: How many times the parameter grid should be redrawn.
        >cv: Cross-validation to be used in determining best parameters (if this argument not passed, then StratifiedKFold
                CV is used with number of folds set to par_n_folds)
        >par_n_folds: Number of folds for StratifiedKFold CV.
                        This argument only considered if the cv argument is not passed (or cv=None is passed).
        >probability: Set to true if you want a probabilistic classifier returned (only works for SVM's).


    OUTPUT:
        > An SKLearn estimator object representing the best estimator with the best parameters."""

    #Check if we should use default value of cv. (Note: don't use the n_folds keyword argument here since it causes problem with the grid's old version of Scikit Learn)
    if cv == None:
        cv = StratifiedKFold(train_target, par_n_folds)

    #Reformat the training data if it has just 1 feature, since it causes cross_val_score(.) to crash
    ...
    #iteration where best classifier was found (this will let the user know if the extra iterations are helping)
    best_iter = 0
    ...
    if 'svm_rbf' in clf_list or 'all' in clf_list:
        ...
        #######I've stopped scaling the data, because it finds a classifier appropriate the scaled data, but then outside this
        #######function, the data is not necessarily scaled, so you run into issues.  I'm not sure how important scaling is anyway...
        ##scaled_train_data = preprocessing.scale(train_data)

        #We now set the initial parameters to search on. Note: these represent powers
        #so C_num values of C are logarithmically spaced between 10^(C_min) and 10^(C_max)
        #I got these particular bounds from: http://scikit-learn.org/0.11/auto_examples/svm/plot_svm_parameters_selection.html
        ...
        #Initialize this classifier's score to zero (as opposed to overall best score)
        this_clf_score = 0
        this_clf_best = None
        this_best_iter = 0
    ...
    if 'svm_sigmoid' in clf_list or 'all' in clf_list:
        ...
        for i in range(iterations):
            ...
            #set the parameter grid
            C_range = np.logspace(C_min, C_max, C_num)
            degree_range = np.linspace(degree_min, degree_max, degree_num)  # linearly space this
            coef0_range = np.logspace(coef0_min, coef0_max, coef0_num)
    ...
    if 'log_reg' in clf_list or 'all' in clf_list:
        ...
        C_max = 7.
        C_num = 4
        #Set intercept_scaling initial parameters (this range is a total guesses!)
                    ...
                    for dual in [False, True]:
                        for fit_intercept in [False, True]:
                            for intercept_scaling in intercept_scaling_range:
                                ...
                                if fit_intercept == False and intercept_scaling in intercept_scaling_range[1:]:
                                    continue
                                #penalty='l1' is apparently only supported when dual is False
                                if penalty == 'l1' and dual != False:
                                    continue
    ...
    if best_clf == None:
        print("\nERROR: No classifiers were found.  Are you sure that your classifier list ('" + str(clf_list) + "') contained a valid classifier (such as 'svm_rbf')?")
        print("Exitting...")
        exit()

    if verbose:
        print("For this training set, the best classifier scored: " + str(best_score) + ", was found on iteration number: " + str(best_iter+1) + " of adaptive parameter search and classifier was: " + str(best_clf))
...
##The below function was rewritten as above to return just the indices which is
##more useful generally than actually applying these to the training data and targets.
##def equalize_training_set(train_data, train_target):
##    """Make training set have an equal number of each class in it by discarding
##        datapoints from over-represented classes.  The particular datapoints to
##        discard are chosen randomly (seeded from /dev/urandom, if available, or the clock)."""
##
##    #First turn targets into a nice form (ie counting from 0 up)
##    class_labels, target = np.unique( train_target, return_inverse=True)
##    num_classes = len(class_labels)
...
##    smallest_cl_size = np.min( np.bincount( target ))
##
##    #indices to be used in equalized
##    indices=[]
##
##    for class_i in range(num_classes):
##
...
##        temp_indices = np.random.permutation( temp_indices )
##        #add some of these indices to our indices list
##        indices.extend( list( temp_indices[:smallest_cl_size] ) )
##
##    #initialize the arrays to be returned
##    num_train_points = num_classes * smallest_cl_size
##    num_feat = train_data.shape[1]
##    eq_train_data = np.zeros((num_train_points, num_feat))
##    eq_train_target = np.zeros(num_train_points)
##
##    #create the equalized training data and target
##    for i in range( len(indices) ):
##
##        eq_train_data[i,:] = train_data[indices[i],:]
##        eq_train_target[i] = train_target[indices[i]]


#####diagnostic stuff#####
#train_data = np.array([[1,2],[2,4],[3,6],[4,8],[5,10],[6,12],[7,14],[8,16],[9,18],[10,20],[11,22],[12,24],[13,26]])
#train_target = np.array([1,2,2,2,2,1,3,3,3,2,3,1,3])
#eq_train_data, eq_train_target = equalize_training_set(train_data, train_target)

#from sklearn import datasets
#iris = datasets.load_iris()
#test_feat_perf(iris.data, iris.target, clf_list=['svm_rbf'], cv_eval=None, par_n_folds=5, iterations=2, equalize_training=True, verbose=True)
